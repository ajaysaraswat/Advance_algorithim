...
#include <fstream> // file handling // isopen
#include <vector> // dynamic array that can grow or shrink 
#include <set> // store unique elements in sorted order // states,alphabets, etc
#include <map> // store key value pair // for transition function
#include <queue> // for FIFO // used for e closure,
#include <string> // store and process input string 
#include <sstream> // reading data from file input as tokens

using namespace std;

// Structure to represent an NFA
struct NFA {
    set<int> states;                  // Q: set of states // unique
    set<char> alphabet;               // Σ: input alphabet (excluding e)
    map<pair<int, char>, set<int>> transitions; // δ: transition function // from this state with this symbol goes to this state // because multiple states to reach with single pair
...
struct Path {
    vector<int> states; // hold the states visited along the path
    vector<char> symbols; // hold the symbols that caused transactions
...
set<int> epsilonClosure(const NFA& nfa, int state) { // return set of all state reachable from 'state' using only e, arg-> ref to nfa(?) struct, state for which want to compute
    set<int> closure = {state}; // set of state that can be reached by epsilon, contain same state initially
    queue<int> queue; // for bfs to explore all e transaction
    queue.push(state); // state pushed in queue for start processing
    ...
    while (!queue.empty()) {
        ...
        auto it = nfa.transitions.find({currentState, 'e'});
...
bool simulateNFA(const NFA& nfa, const string& input, vector<Path>& allPaths) {
    ...
    for (int state : currentStates) {
        ...
        if (state != nfa.startState) {
            initialPath.symbols.push_back('e');
                ...
                for (int nextState : it->second) {
                    ...
                    for (int epsilonState : epsilonStates) {
                        if (epsilonState != nextState) {
                            ...
                            epsilonPath.symbols.push_back('e');
